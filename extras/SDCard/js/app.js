...
async function loadMacros() {
    try {
        ...
        if (data.macros) {
            // Issue #9: the list already carries id, name and duration -
            // no per-macro detail fetch. Full details are loaded lazily by
            // editMacro() only for the macro the user actually opens.
            state.macros = data.macros.map((macro) => ({
                ...macro,
                duration: (macro.duration != null) ? macro.duration : 3600,
            }));
            renderMacroList();
