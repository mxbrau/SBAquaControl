...
function showTimeSetDialog() {
    // Get current time and populate the display
    ...
    // Pre-fill the input with current time if available
    if (currentTime && currentTime !== '--:--:--') {
        document.getElementById('timeSetInput').value = currentTime;
    } else {
        // Default to current browser time
        const now = new Date();
        const timeStr = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')}`;
        document.getElementById('timeSetInput').value = timeStr;
    }
    
    // Show modal
    document.getElementById('timeSetModal').classList.remove('hidden');
}

function closeTimeSetDialog() {
    document.getElementById('timeSetModal').classList.add('hidden');
}

async function syncTimeWithBrowser() {
    const now = new Date();
    ...
    try {
        const result = await API.setTime(hour, minute, second);
        console.log('✅ Time synchronized with browser:', result.time);
        ...
        alert(`✅ Zeit erfolgreich auf ${result.time} gesetzt (synchronisiert mit Browser)`);
    } catch (error) {
...
async function setTimeManually() {
    const timeInput = document.getElementById('timeSetInput').value;
    ...
    // Parse time input (format: HH:MM:SS or HH:MM)
    ...
    try {
        const result = await API.setTime(hour, minute, second);
        console.log('✅ Time set manually:', result.time);
        ...
        console.error('❌ Time set failed:', error);
        alert(`Fehler beim Setzen der Zeit: ${error.message}`);
    }
}
